            ...
            :mpBMap(pBMap),mpCC(pCC),mbDrawFrame(false)
    {
        ...
        if(mpCC->mSysState == eSystemState::CLIENT)
        {
            ...
            *ss << "ImageClient" << mpCC->mClientId;
            ...
            {
                ss = new stringstream;
                *ss << "ClientMarkerMap" << mpBMap->mBMapId;
                mPubMarker0 = mNh.advertise<visualization_msgs::Marker>(ss->str(),10);

                ss = new stringstream;
                *ss << "ClientMapPointsMap" << mpBMap->mBMapId;
                mPubPcl0 = mNh.advertise<sensor_msgs::PointCloud2>(ss->str(),10);

                ss = new stringstream;
                *ss << "LocalMPs" << mpBMap->mBMapId;
                mPubLocalMPs = mNh.advertise<sensor_msgs::PointCloud2>(ss->str(),10);
            }
            else if(mpBMap->mBMapId == 1)
            {
                ss = new stringstream;
                *ss << "ClientMarkerMap" << mpBMap->mBMapId;
                mPubMarker1 = mNh.advertise<visualization_msgs::Marker>(ss->str(),10);
    ...
    void Viewer::PubTrajectories()
    {
        ...
        for(int ito = 0;ito<4;++ito)
        {
            ...
            for(int iti = 0;iti<=vMaxId[ito];++iti)
            {
                ...
                rotation_matrix<<T.at<float>(0,0),T.at<float>(0,1),T.at<float>(0,2),
                ...
                if (!fout_traj)
                {
                    ...
                    if(iti==vMaxId[ito])
                    {
                        ...
                        fout_traj << p.y<<" ";
                        fout_traj << p.z<<" ";
                        fout_traj << quaternion.x()<<" ";
                        fout_traj << quaternion.y()<<" ";
                        fout_traj << quaternion.z()<<" ";
                        fout_traj << quaternion.w()<<endl;
                    }
