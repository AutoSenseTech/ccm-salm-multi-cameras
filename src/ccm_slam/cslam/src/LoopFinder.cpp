...
void LoopFinder::Run()
{
    while(1)
    {
        ...
        #endif
        {
            //cout <<"线程2:进入loop finder" <<endl;

            while(!mpCC->LockPlaceRec()){usleep(params::timings::miLockSleep);}

            // Detect loop candidates and check covisibility consistency
            //cout <<"线程2:loopfinder DetectLoop" <<endl;
            bool bDetect = DetectLoop();

            if(bDetect)
            {
                // Compute similarity transformation [sR|t]
                //cout <<"线程2:loopfinder ComputeSim3" <<endl;
                bool bSim3 = ComputeSim3();
                if(bSim3)
                {
                    // Perform loop fusion and pose graph optimization
                    //cout <<"线程2:loopfinder CorrectLoop" <<endl;
                    CorrectLoop();
                }
            }
            mpCC->UnLockPlaceRec();
...
//    g2o::Sim3 g2oScur(Converter::toMatrix3d(Rcur),Converter::toVector3d(tcur),1.0);
    ...
    //cout<<"++++++++开始loopfinderGBA，新地图时间： "<<setprecision(25)<<mpCurrentKF->mTimeStamp<<endl;
...
//        mpMap->LockMapUpdate()
        ...
        cout << "-> Global Bundle Adjustment finished(Loop Finder)" << endl;
        ...
        cout << "+++++GBA坏掉的帧数(Loop Finder)： "<<nBadGBA<<endl;
