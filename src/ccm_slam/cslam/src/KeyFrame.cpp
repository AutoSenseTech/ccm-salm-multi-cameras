...
*/
//少了双目里面中的部分东西，如昂反投影计算空间三维点，这个函数用在了初始化部分
...
KeyFrame::KeyFrame(Frame &F, mapptr pMap, dbptr pKFDB, commptr pComm, eSystemState SysState, size_t UniqueId) //添加五个双目成员变量的初始化，mbf,mb,mThDepth,mvDepth,mvuRight
    : mFrameId(F.mId),mUniqueId(UniqueId),mTimeStamp(F.mTimeStamp),mVisId(-1),
      ...
      N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn), mDescriptors(F.mDescriptors.clone()),mvuRight(F.mvuRight), mvDepth(F.mvDepth),
...
//for multi-camera
KeyFrame::KeyFrame(Frame &F, const int &id, const int &cameraId, eSystemState SysState, size_t UniqueId)
    :cameraNum(F.cameraNum), mFrameId(F.mId), mUniqueId(UniqueId), mTimeStamp(F.mTimeStamp), mVisId(-1),
     mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
    mfGridElementWidthInv(F.mvGridElementWidthInv[cameraId]), mfGridElementHeightInv(F.mvGridElementHeightInv[cameraId]),
    mTrackReferenceForFrame(defpair), mFuseTargetForKF(defpair), 
    mBALocalForKF(defpair), mBAFixedForKF(defpair),
    mLoopQuery(defpair), mnLoopWords(0), mRelocQuery(defpair), mnRelocWords(0), mBAGlobalForKF(defpair),
    fx(F.vfx[cameraId]), fy(F.vfy[cameraId]), cx(F.vcx[cameraId]), cy(F.vcy[cameraId]), invfx(F.vinvfx[cameraId]), invfy(F.vinvfy[cameraId]),mThDepth(F.mThDepth),
    N(F.mvpkeyPointsNum[cameraId]), mvKeys(F.mvKeysMultiple[cameraId]), mvKeysUn(F.mvKeysMultipleUn[cameraId]), mDescriptors(F.mvpDescriptors[cameraId].clone()),
    mnScaleLevels(F.mnScaleLevels), mfScaleFactor(F.mfScaleFactor),
    mfLogScaleFactor(F.mfLogScaleFactor), mvScaleFactors(F.mvScaleFactors), mvLevelSigma2(F.mvLevelSigma2),
    mvInvLevelSigma2(F.mvInvLevelSigma2), mnMinX(F.mvMinX[cameraId]), mnMinY(F.mvMinY[cameraId]), mnMaxX(F.mvMaxX[cameraId]),
    mnMaxY(F.mvMaxY[cameraId]), mK(F.mvpK[cameraId]), 
    mpORBvocabulary(F.mpORBvocabulary), mbFirstConnection(true), mpParent(nullptr), mbNotErase(false),
    mbToBeErased(false), mbBad(false), mHalfBaseline(0), mbIsEmpty(false),mbPoseLock(false),mbPoseChanged(false),mbInOutBuffer(false), mbSentOnce(false),
      mMatchQuery(defpair),
      mSysState(SysState),mbOmitSending(false),
      mbLoopCorrected(false),
      mbAck(false),mbFromServer(false),mbUpdatedByServer(false),mCorrected_MM(defpair),mbSendFull(true)
{
    cout<<"enter keyframe: "<<cameraId<<endl;
    if(cameraId == 0)
    {
    ...
    else if(cameraId == 1)
    {
        ...
        mId =make_pair(id,F.mId.second);
        mTrl = F.mvTcamji[cameraId-1];  //Tii-1
        mTi0 = F.mvTcamji[0];
        mGrid.resize(mnGridCols);
        for(int i=0; i<mnGridCols;i++)
        {
            mGrid[i].resize(mnGridRows);
            for(int j=0; j<mnGridRows; j++)
                mGrid[i][j] = F.mGrid1[i][j];
        }

        SetPose(mTi0 *F.mTcw, false);
    }
    else if(cameraId == 2)
    {
        mId =make_pair(id,F.mId.second);
        mTrl = F.mvTcamji[cameraId-1];
        mTi0 = F.mvTcamji[1]*F.mvTcamji[0];
        mGrid.resize(mnGridCols);
        for(int i=0; i<mnGridCols;i++)
        {
            mGrid[i].resize(mnGridRows);
            for(int j=0; j<mnGridRows; j++)
                mGrid[i][j] = F.mGrid2[i][j];
        }
        SetPose(mTi0 *F.mTcw,false);
    }
    else
    {
       mId =make_pair(id,F.mId.second);
        mTrl = F.mvTcamji[cameraId-1];
        mTi0 = F.mvTcamji[2]*F.mvTcamji[1]*F.mvTcamji[0];
        mGrid.resize(mnGridCols);
        for(int i=0; i<mnGridCols;i++)
        {
            mGrid[i].resize(mnGridRows);
            for(int j=0; j<mnGridRows; j++)
                mGrid[i][j] = F.mGrid3[i][j];
        }
        SetPose(mTi0 *F.mTcw,false);
    }
    //mvbMapPointsLock.resize(N,false);
...
cv::Mat KeyFrame::UnprojectStereo(int i)
{
    ...
    if(z>0)
    {
        ...
        // if(mvKeys.empty()){
        //     cout<<"UnprojectStereo error ID: "<<endl;
        // }
...
void KeyFrame::ReduceMessage(ccmslam_msgs::KF *pMsgFull, ccmslam_msgs::KFred *pMsgRed)
{
    ...
    //新增 TODO: 这个vector 是不是要用for 循环
    pMsgRed->mbf = pMsgFull->mbf;
    pMsgRed->mThDepth = pMsgFull->mThDepth;
    // pMsgRed->mvDepth = pMsgFull->mvDepth;
    // pMsgRed->mvuRight = pMsgFull->mvuRight;
}
...
void KeyFrame::WriteMembersFromMessage(ccmslam_msgs::KF *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    ...
    //新增
    mvDepth.resize(pMsg->mvDepth.size());
    for(int idx=0;idx<pMsg->mvDepth.size();++idx)
        mvDepth[idx] = pMsg->mvDepth[idx];

    mvuRight.resize(pMsg->mvuRight.size());
    for(int idx=0;idx<pMsg->mvuRight.size();++idx)
        mvuRight[idx] = pMsg->mvuRight[idx];
