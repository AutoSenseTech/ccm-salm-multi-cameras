...
public:
    ...
    KeyFrame(Frame &F, const int &id, const int &cameraId, eSystemState SysState, size_t UniqueId);
    ...
    void AddMapPoint(mpptr pMP, const size_t &idx, bool bLock = false); //多了第三个判断的bool值
    void EraseMapPointMatch(const size_t &idx, bool bLock = false);
    void EraseMapPointMatch(mpptr pMP, bool bLock = false);
    void ReplaceMapPointMatch(const size_t &idx, mpptr pMP, bool bLock = false, bool bOverrideLock = false);//多了第三和第四个判断的bool值
    ...
    cv::Mat UnprojectStereo(int i); // 添加双目反投影的函数
...
public:
    ...
    std::vector<float> mvDepth; // negative value for monocular point
...
protected:
    ...
    std::vector<mpptr> mvpMapPoints;  // by the order of keypoint ID
