...
public:
    ...
    void static LocalBundleAdjustmentClient(kfptr pKF, bool *pbStopFlag, mapptr pMap, size_t ClientId, eSystemState SysState = eSystemState::CLIENT); //添加了后两个参数

    void static LocalBundleAdjustmentClient(bkfptr pBKFs, bool *pbStopFlag, bmapptr pBMap, size_t ClientId, eSystemState SysState = eSystemState::CLIENT);
    
    int static PoseOptimizationClient(Frame &Frame);

    //for multi camera
    int static PoseOptimizationClientPlus(Frame &pFrame);
    //-----------server-----------
