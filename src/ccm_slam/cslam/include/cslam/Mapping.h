...
public:
    //constructor
    int count = 0; //culling的次数
    LocalMapping(ccptr pCC, mapptr pMap, dbptr pDB, viewptr pViewer);
    LocalMapping(ccptr pCC, bmapptr pBMap, bdbptr pBDB, viewptr pViewer);
    ...
    void InsertBundledKeyFrames(bkfptr pBKFs);
    ...
    void SetAcceptBundledKeyFrames(bool flag);
    ...
    int BundledKeyframesInQueue(){
        unique_lock<std::mutex> lock(mMutexNewBKFs);
        return mlNewBundledKeyFrames.size();
    }

protected:
    bool mbMonocular = false; //判断输入的是monocular还是stereo

    void KeyFrameCulling();
    bool CheckNewKeyFrames();
    bool CheckNewBundledKeyFrames();
    void ProcessNewKeyFrame();
    void ProcessNewBundledKeyFrames();
    ...
    std::list<bkfptr> mlNewBundledKeyFrames;
    std::list<kfptr> mlKfsForLBA;
    kfptr mpCurrentKeyFrame;
    bkfptr mpCurrentBundledKeyFrames;
    ...
    bool mbAcceptBundledKeyFrames;
    //mutexes
    std::mutex mMutexAccept;
    std::mutex mMutexStop;
    std::mutex mMutexNewKFs;
    std::mutex mMutexNewBKFs;
